...
namespace PruneUrl.Backend.Infrastructure.IoC.Tests.IntegrationTests.Extensions
{
  ...
  public sealed class ContainerBuilderExtensionsIntegrationTests
  {
    ...
    public void RegisterAllModulesTest()
    {
      ...
      {
        SequenceIdOptions optionsObj = configuration.GetSection(nameof(SequenceIdOptions)).Get<SequenceIdOptions>() ?? throw new Exception($"Missing config for {nameof(SequenceIdOptions)}!");
        return Options.Create(optionsObj);
      });
      containerBuilder.Register(componentContext =>
      {
        FirestoreTransactionOptions optionsObj = configuration.GetSection(nameof(FirestoreTransactionOptions)).Get<FirestoreTransactionOptions>() ?? throw new Exception($"Missing config for {nameof(FirestoreTransactionOptions)}!");
        return Options.Create(optionsObj);
      });
      ...
      using (IContainer container = containerBuilder.Build())
      {
        Assert.Multiple(() =>
        {
          ...
          Assert.That(container.Resolve<IDbTransactionProvider>(), Is.TypeOf<FirestoreDbTransactionProvider>());
          Assert.That(container.Resolve<IDbWriteBatchFactory>(), Is.TypeOf<FirestoreDbWriteBatchFactory>());
          ...
          Assert.That(container.Resolve<MapperConfiguration>().AssertConfigurationIsValid, Throws.Nothing);
        });
      }
    }
